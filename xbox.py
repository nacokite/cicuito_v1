...
class Joystick :
    ...
    def __init__(self, idjos):
        self.numJos = idjos
        if self.numJos >=0:
            print (self.numJos)
            
            cmd = 'sudo xboxdrv --wid ' + str(self.numJos) + ' --detach-kernel-driver --next-controller'
            print (cmd)
            self.proc = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, bufsize=0)
            #~ sudo xboxdrv --wid 0 --led 2 --no-uinput --detach-kernel-driver
        #~ if self.numJos == 100:
            #~ cmd = ['xboxdrv','--wid 1','--led 3','--no-uinput','--detach-kernel-driver']    
            #~ self.proc = subprocess.Popen(['sudo xboxdrv','--wid 1','--detach-kernel-driver'], shell=True, stdout=subprocess.PIPE, bufsize=0)
            
        #~ self.proc = subprocess.Popen(['xboxdrv', '--no-uinput','--detach-kernel-driver'], stdout=subprocess.PIPE, bufsize=0)
        #~ self.proc = subprocess.Popen(['xboxdrv','--wid 0','--led 2','--no-uinput','--detach-kernel-driver'], shell=True,stdout=subprocess.PIPE, bufsize=0)
        ...
        self.refreshDelay = 1.0 / 30   #joystick refresh is to be performed 30 times per sec by default
        #
